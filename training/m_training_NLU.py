...
from model.model_nlu import NLU_Encoder, NLU_Decoder, NLU_Model
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d_model', help='output model directory', default='../output/NLU/')
    ...
    optimizer = torch.optim.Adam(nlu_model.parameters(), lr = args.lr)
    criterion_mr_value = nn.CrossEntropyLoss()
    ...
    parameters = {
        ...
        'parameters': count_parameters(nlu_model),
        'training': {
            'epoch': args.epoch,
            'batch': args.batch,
            'lr': args.lr,
    ...
    for epoch in range(args.epoch):
        ...
        loss_train = train.train_nlu(nlu_model, dataloader_train, optimizer, criterion_mr_value, args.clip, device, args.v)
        loss_valid = train.evaluate_nlu(nlu_model, dataloader_valid, criterion_mr_value, device)
        loss_test = train.evaluate_nlu(nlu_model, dataloader_test, criterion_mr_value, device)
        ...
        if best_loss_valid > loss_valid:
            ...
            torch.save({
                ...
                'model_dict': nlu_model.state_dict(),
                'model': nlu_model},
